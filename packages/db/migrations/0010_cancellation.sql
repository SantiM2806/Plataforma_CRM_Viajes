-- ============================================================
-- 0010_cancellation.sql
-- Política de cancelación en opciones y reservas (para seguimiento de pagos).
-- ============================================================
begin;

alter table quote_options add column if not exists refundable boolean;
alter table quote_options add column if not exists free_cancellation_until timestamptz;

alter table reservations add column if not exists refundable boolean;
alter table reservations add column if not exists free_cancellation_until timestamptz;

-- Recrear decide_public_quote para copiar la cancelación a la reserva.
  ...
  if p_decision = 'approve' then
    ...
    if not exists (select 1 from reservations where quote_id = v_quote.id) then
      insert into reservations (
        ...
        refundable, free_cancellation_until
      )
      select
        ...
        o.refundable, o.free_cancellation_until
